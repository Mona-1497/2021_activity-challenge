...
titleLb = Label(root, text='Your Group Shared Activities', font=("Comic Sans MS", 20), bg='#064134', fg='orange')
titleLb.place(x=360,y=15)
...
def addQuestion(result,root1,question_label,activityID,speakbtn,ans1,ans2,ans3,statelb,correctans,ansimg):
    ...
    if i >= len(result):
        ...
        Label(root2, image=photo2, bg='#064134',bd=0).place(x=400,y=80)
        def callback4():
            root2.destroy()
            os.system('MainMenu.py')
...
sql="Select ActID from sharedAct Where groupName= %s"
sqlfind="Select * from addactivity Where id= %s"
sql3="SELECT groupName FROM joinToGroup Where memberName=%s"
#sql5="SELECT * FROM addactivity Where id=%s"
mycursor.execute(sql3,(current_user,))
res = mycursor.fetchall()
n = StringVar()
groupchoosen = ttk.Combobox(root, width=20,textvariable=n)
groupchoosen.place(x=450,y=100)
...
Button(root,text='Select and start activity',image=sel,compound=LEFT, bg='#064134', bd=0,font=("bold",12),fg='white',command=select).place(x=400,y=450)
def groupAct(groupName):
    indexlb.config(text='',image='')
    Lb.delete(0,END)
    mycursor.execute(sql,(groupName,))
    myresult = mycursor.fetchall()
    for i in range (len(myresult)):
         mycursor.execute(sqlfind, (myresult[i][0],))
         resqlfind =mycursor.fetchall()
         Lb.insert(END,resqlfind[i])
...
Button(root,image=serch,text="search!",compound=LEFT,font=("bold",12),fg='white', bg='#064134',bd=0,command=lambda :groupAct(groupchoosen.get())).place(x=450,y=140)
